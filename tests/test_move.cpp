...
void game1() {
    ...
    for (std::string_view move_str : game1_record) {
        auto move = Move::parse_san(move_str, pos);
        if (!move) {
            std::cout << "failed to parse \"" << move_str << "\" in " << pos << std::endl;
        }
        REQUIRE(move);
    ...
    REQUIRE(to_fen(pos) == "8/5pk1/7p/8/1p4P1/1P1R2P1/3N1qBP/3Nr2K w - - 1 41");
}
...
void cases() {
    std::vector<std::tuple<std::string_view, std::string_view, std::string_view>> cases{{
      ...
      // Push promotion
      {"8/4P3/8/8/8/8/8/k1K5 w - - 0 1", "e8=Q", "4Q3/8/8/8/8/8/8/k1K5 b - - 0 1"},
      // Capture underpromotion
      {"3n4/4P3/8/8/8/8/8/k1K5 w - - 0 1", "exd8=N", "3N4/8/8/8/8/8/8/k1K5 b - - 0 1"},
      // En passant
      {"k1K5/8/8/8/4Pp2/8/8/8 b - e3 0 1", "fxe3", "k1K5/8/8/8/8/4p3/8/8 w - - 0 2"},
      // Double pawn push (creates en passant square)
      {"k1K5/8/8/8/3p4/8/4P3/8 w - - 0 1", "e4", "k1K5/8/8/8/3pP3/8/8/8 b - e3 0 1"},
    }};
    for (auto [before, san, after] : cases) {
        auto pos1 = Position::parse(before);
        REQUIRE(pos1);
        auto move = Move::parse_san(san, *pos1);
        if (!move) {
            std::cout << "failed to parse \"" << san << "\" in " << *pos1 << std::endl;
        }
        REQUIRE(move);
        Position pos2 = pos1->move(*move);
        if (to_fen(pos2) != after) {
            std::cout << "expected " << after << ", got " << pos2 << std::endl;
        }
        REQUIRE(to_fen(pos2) == after);
    }
}
...
void invalid() {
    Position startpos =
      Position::parse("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1").value();
    for (std::string_view san : {"e5", "Nc6", "exd3", "Ke2", "Qxd7", "Nfe2"}) {
        REQUIRE(!Move::parse_san(san, startpos));
    }
}
