...
PScore evaluate_king_safety(const Position& pos) {
    ...
    Bitboard our_double_pawn_defended = static_pawn_double_attacks<color>(pos.bitboard_for(color, PieceType::Pawn));

    Bitboard king_ring     = king_ring_table[pos.king_sq(color).raw] & ~our_double_pawn_defended;
    Bitboard extended_ring = extended_ring_table[pos.king_sq(color).raw] & ~our_double_pawn_defended;
