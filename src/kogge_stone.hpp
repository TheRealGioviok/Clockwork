...
sliders_setwise(Bitboard src1, Bitboard src2, Bitboard occ) {
    ...
    // 2 <=> !a & !b & c
    gen = _mm512_ternarylogic_epi64(_mm512_set1_epi64(occ.value()), mask,
                                    _mm512_rolv_epi64(gen, shift1), 2);
...
#else
    constexpr u64x4 shift1{
      std::array<u64, 4>{orth1 ? 8 : 7, orth1 ? 1 : 9, orth2 ? 8 : 7, orth2 ? 1 : 9}};
    constexpr u64x4 shift2 = shift1 + shift1;
    constexpr u64x4 shift4 = shift2 + shift2;
    ...
    gen1 = (gen1 >> shift1) & ~filemask1 & ~u64x4::splat(occ.value());
    gen2 = (gen2 << shift1) & ~filemask2 & ~u64x4::splat(occ.value());
...
static inline Bitboard knights_setwise(Bitboard knights, Bitboard occ) {
#ifdef LPS_AVX512
    ...
    return Bitboard{static_cast<u64>(attacks)} & ~occ;
#else
    ...
    return Bitboard{(lo | hi).reduce_or()} & ~occ;
#endif
}
