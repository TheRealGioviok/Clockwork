...
int main() {
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (size_t bi = 0, bstart = 0; bstart < positions.size(); bstart += batch_size, ++bi) {
            ...
            const f64 running_loss_here =
              running_loss_accum.exchange(0.0, std::memory_order_relaxed);

            print_progress(bi + 1, total_batches, running_loss_here);
        }
...
void print_params() {
    ...
    std::cout << "inline const PParam RESTRICTED_SQUARES = " << RESTRICTED_SQUARES << ";"
              << std::endl;
    std::cout << "inline const PParam LATENT_INVASION_SQUARES = " << LATENT_INVASION_SQUARES << ";"
              << std::endl;
